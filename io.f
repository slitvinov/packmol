...
c
      ...
      subroutine getinp(dism,precision,
     +                  ntype,nlines,nrest,
...
c
     ...
     +                 xzyy, xzyz, xyyx, xq, yq, zq, q0, q1, q2, q3
      ...
      integer k, ilugan, ilubar, icart, itype, imol, idatom,
     +        irest, iimol, ichain, iatom, irec, ilres, ifres,
     +        icount, iires, ifmol, iconn, charl 
...
c The same thing for fixed molecules
...
c write the output as pdb file
  ...
  856     format('CRYST1',t7,f9.2,t16,f9.2,t25,f9.2,
     ...
     +                    t56,'P 1           1')
        ...
        do itype = 1, ntype
          ...
          ntotres = ntotres - iimol

          open(15,file=pdbfile(itype), status = 'old')
          do irec = 1, 80
            record(irec:irec) = ' '
          end do
  ...
  920       format(a80)
          ...
          if(record(1:4).eq.'ATOM') then
            error = .true.
            read(record(23:26),*,err=921,end=921) ifres
            error = .false.
  921       if(error) then
              record = pdbfile(itype)
              write(*,*) ' ERROR: Failed reading residue number from',
     +                   ' PDB file: ', record(1:charl(record))
              write(*,*) ' Residue numbers are integers that must be',
     +                   ' between columns 23 and 26. '
              write(*,*) ' Other characters within these columns will',
     +                   ' cause input/output errors. '
              write(*,*) ' Standard PDB format specifications can be',
     +                   ' found at: '
              write(*,*) ' www.rcsb.org/pdb '
              stop
            end if          
          else if(record(1:6).eq."HETATM") then
            read(record(23:26),*,err=922,end=922) ifres
  922       continue
          end if
          do while(record(1:4).eq.'ATOM')
            read(15,920,err=925,end=925) record
            read(record(23:26),*,err=925,end=925) ilres
          end do
  925     continue
          nres = ilres - ifres + 1
          close(15)
     ...
     +             .and.record(1:6).ne.'HETATM')
          ...
          idatom = idfirst(itype) - 1     
          iatom = 0
  857     do while(iatom.lt.natoms(itype))
            iatom = iatom + 1 
...
c Setting residue numbers for this molecule
  ...
  915       continue
            if(resnumbers(itype).eq.0) then
              iires = mod(imol,9999)
            else if(resnumbers(itype).eq.1) then
              iires = imark
            else if(resnumbers(itype).eq.2) then
              iires = mod(iimol+imark-ifres,9999)
            end if
...
c Writing output line
            ...
            if(record(1:4).eq.'ATOM') then
              write(30,930) record(1:5), icart - natfix,
     +                      record(12:21), chain, iires, 
     +                      record(27:27),
     +                      (xcart(icart,k), k = 1, 3),
     +                      record(55:80)
  930                       format(t1,a5,t7,i5,t12,a10,t22,a1,t23,i4,
     +                      t27,a1,t31,f8.3,t39,f8.3,t47,f8.3,t55,a26) 
            end if

               if(record(1:6).eq.'HETATM') then
                 write(30,931) record(1:6), icart - natfix,
     +                           record(12:21), chain, iires, 
     +                           record(27:27),
     +                           (xcart(icart,k), k = 1, 3),
     +                           record(55:80)
  931                            format(t1,a6,t7,i5,t12,a10,t22,a1,
     +                                  t23,i4,t27,a1,t31,f8.3,t39,f8.3,
     +                                  t47,f8.3,t55,a26) 
            end if
  ...
  859     continue
     ...
     +             .and.record(1:6).ne.'HETATM')
          ...
          if(natoms(itype).eq.1) close(15)

          ilugan = ilugan + 3 
          ilubar = ilubar + 3 
 
          ntotres = ntotres + nres
  ...
  932     format('TER')
     ...
     +             .and.record(1:6).ne.'HETATM')
          ...
          call chainc(ichain,chain)
          if(record(1:4).eq.'ATOM') then
            read(record(23:26),*) ifres
          else if(record(1:6).eq."HETATM") then
            read(record(23:26),*,err=939,end=939) ifres
  939       continue
          end if
          do while(record(1:4).eq.'ATOM')
            read(15,920,err=940,end=940) record
            read(record(23:26),*,err=940,end=940) ilres
          end do
  940     continue
          nres = ilres - ifres + 1
          close(15)
     ...
     +             .and.record(1:6).ne.'HETATM')
          ...
          iatom = 0
  941     do while(iatom.lt.natoms(itype))
            iatom = iatom + 1
            idatom = idatom + 1
            icart = icart + 1
     ...
     +        read(15,858,err=933,end=933) record
            ...
            if(record(1:4).eq.'ATOM') then
              read(record(23:26),*) icount
              if(resnumbers(itype).eq.0) then
                iires = 1
              else if(resnumbers(itype).eq.1) then
                iires = icount
              else if(resnumbers(itype).eq.2) then
                iires = mod(iimol+icount-ifres,9999) 
              end if
              write(30,930) record(1:5), icart,
     ...
     +                         record(55:80)
            ...
            if(record(1:6).eq.'HETATM') then
              ...
              if(resnumbers(itype).eq.0) then
                ...
                iires = mod(iimol+icount-ifres-1,9999) 
              end if
              write(30,931) record(1:6), icart, 
     +                      record(12:21), record(22:22), iires,
     +                      record(27:27),
     +                      (coor(idatom,k), k = 1, 3),
     +                      record(55:80)
